...
backtest_90pct.py
=================
90% Confidence Backtest Engine â€” SPY options 2021-2026.
Based on backtest_real_data.py with 5 upgrades:

  1. Full Commission Model  â€” realistic total ~$1.20-1.25/contract/leg
  2. Improved Slippage      â€” 15% worse than bid/ask on all fills
  3. Parameter Stability    â€” S1 sensitivity sweep (VIX/delta/entry time)
  4. Crash Period Analysis  â€” 2018 Q4, 2020 March, 2022 bear, 2024 Aug
  5. Confidence Score       â€” 0-100 composite per strategy

Strategy logic is IDENTICAL to backtest_real_data.py.
Output: backtest_90pct_report.txt | backtest_90pct_trades.csv
"""

import csv, math, os, pickle, random, sys, time as _time, traceback, warnings
...
REPORT_PATH = Path(r'C:\Users\sagla\spy-0dte-trader\backtest_90pct_report.txt')
TRADES_PATH = Path(r'C:\Users\sagla\spy-0dte-trader\backtest_90pct_trades.csv')
...
# â”€â”€ backtest period â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€ commission model (UPGRADE 1) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
COMMISSION_BASE  = 0.65
SEC_FEE_RATE     = 0.000008
FINRA_FEE_RATE   = 0.000119
EXCHANGE_FEE     = 0.10

def _commission_per_leg(contracts: int, spy_price: float) -> float:
    """Realistic commission per contract per leg."""
    return (COMMISSION_BASE * contracts
            + spy_price * 100 * contracts * SEC_FEE_RATE
            + contracts * 100 * FINRA_FEE_RATE
            + EXCHANGE_FEE * contracts)

# â”€â”€ slippage model (UPGRADE 2) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
def _fill_buy(bid: float, ask: float) -> float:
    """Buy fill: 15% worse than ask."""
    return ask + max(ask - bid, 0.0) * 0.15

def _fill_sell(bid: float, ask: float) -> float:
    """Sell fill: 15% worse than bid."""
    return max(bid - max(ask - bid, 0.0) * 0.15, 0.01)

# â”€â”€ crash periods (UPGRADE 4) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
CRASH_PERIODS: Dict[str, tuple] = {
    '2018_Q4':    (date(2018, 10, 1), date(2018, 12, 31), '2018 Q4 Crash (VIX->36, SPY -20%)'),
    '2020_march': (date(2020,  2, 1), date(2020,  4, 30), '2020 COVID Crash (VIX->82, SPY -34%)'),
    '2022_bear':  (date(2022,  1, 1), date(2022, 12, 31), '2022 Bear Market (VIX avg 27, SPY -25%)'),
    '2024_aug':   (date(2024,  8, 1), date(2024,  8, 31), '2024 August VIX Spike (VIX->38)'),
}

EARNINGS_UNIVERSE = ['NVDA', 'TSLA', 'AMD', 'AAPL', 'MSFT', 'META', 'GOOGL', 'AMZN']

_RFR = {2018: 0.020, 2019: 0.024, 2020: 0.010, 2021: 0.001,
        2022: 0.015, 2023: 0.045, 2024: 0.050, 2025: 0.045, 2026: 0.043}

def _rfr(dt) -> float:
    if isinstance(dt, str): dt = date.fromisoformat(dt[:10])
    return _RFR.get(dt.year, 0.045)

# â”€â”€ FOMC dates 2021-2026 â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
FOMC_DATES = {
...
class ThetaCache:
    ...
    def get_quote(self, obs_date: date, expiration: date,
        ...
        """Single quote lookup â€” loads (and caches) the full chain for expiration."""
...
def _synth_5min_from_daily(spy_daily: dict) -> dict:
    result = {}
    for ds, d in spy_daily.items():
        o    = d.get('open',  d.get('close', 500))
        h_val = d.get('high', d.get('close', 500))
        c_val = d.get('close', 500)
        l_val = d.get('low',  d.get('close', 500))
        day = date.fromisoformat(ds)
        bars = []
        for i in range(78):
            t_dt = datetime(day.year, day.month, day.day, 9, 30) + timedelta(minutes=i * 5)
            try:
                t_ms = int(ET.localize(t_dt).timestamp() * 1000)
            except Exception:
                t_ms = int(t_dt.timestamp() * 1000)
            frac  = i / 77.0
            price = round(o + (c_val - o) * frac, 2)
            bars.append({'t': t_ms, 'o': price,
                         'h': round(min(price * 1.001, h_val), 2),
                         'l': round(max(price * 0.999, l_val), 2),
                         'c': price, 'v': 40_000_000 // 78})
        result[ds] = bars
    return result


def _synthetic_expirations(start: date, end: date) -> List[date]:
    result, d = [], start
    while d <= end:
    ...
    def _day_iv(self, obs_date: date, expirations: List[date]) -> float:
        ...
        for exp in expirations:
            ...
            for (K, right), q in chain.items():
                if abs(K - spy_close) > spy_close * 0.015:
                    continue
                mid = (q['bid'] + q['ask']) / 2.0
                if mid <= 0.01:
                    continue
...
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
# 11. STRATEGY 1 â€” Credit Spread (0DTE puts, Mon/Fri, VIX 15-22) â€” parameterized
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•

def run_s1_credit_spread(
        spy_5min: dict, vix_daily: dict, spy_daily: dict,
        pricer: OptionPricer, expirations: List[date],
        use_real: bool, dates: List[str],
        vix_min: float = 15.0, vix_max: float = 22.0,
        target_delta: float = 0.20,
        entry_hour_start: int = 10, entry_minute_start: int = 0,
        entry_hour_end: int = 11) -> List[Trade]:
    ...
    for ds in dates:
        ...
        # Entry window: parameterized
        entry_bar = None
        for bar in bars:
            dt_bar = datetime.fromtimestamp(bar['t'] / 1000, tz=ET)
            if (dt_bar.hour > entry_hour_start or
                    (dt_bar.hour == entry_hour_start and dt_bar.minute >= entry_minute_start)) \
                    and dt_bar.hour < entry_hour_end:
                entry_bar = bar
                break

        if entry_bar is None:
            dbg['no_entry_bar'] += 1
            first_reject.setdefault('no_entry_bar', f'{ds}: no bar in entry window')
            continue
        ...
        # Find target-delta put strike
        short_strike = pricer.strike_for_delta(target_delta, spy, dt_obj, exp, 'P',
                                                expirations, use_real=use_real)
        short_strike = round(short_strike / 0.5) * 0.5
        ...
        # Apply slippage: sell short at slipped bid, buy long at slipped ask
        short_fill = _fill_sell(short_bid, short_ask)
        long_fill  = _fill_buy(long_bid, long_ask)
        credit = round(short_fill - long_fill, 4)
        if credit <= 0.02:
            dbg['low_credit'] += 1
            first_reject.setdefault('low_credit',
                f'{ds}: credit={credit:.4f} short_fill={short_fill:.4f} long_fill={long_fill:.4f}')
            continue
        dbg['entered'] += 1
        ...
        for bar in bars:
            ...
            current_debit = max(round(short_cur_ask - long_cur_bid, 4), 0)
        ...
        if use_real:
            short_exit_bid, short_exit_ask, _, _ = pricer.intraday_price(
                spy_exit, short_strike, ml_exit, 'P', dt_obj, expirations)
            long_exit_bid, long_exit_ask, _, _ = pricer.intraday_price(
                spy_exit, long_strike, ml_exit, 'P', dt_obj, expirations)
        else:
            short_exit_bid, short_exit_ask, _, _ = pricer.synthetic_intraday(
                spy_exit, short_strike, ml_exit, 'P', dt_obj)
            long_exit_bid, long_exit_ask, _, _ = pricer.synthetic_intraday(
                spy_exit, long_strike, ml_exit, 'P', dt_obj)

        # Apply exit slippage: buy-to-close short at slipped ask, sell-to-close long at slipped bid
        short_exit_fill = _fill_buy(short_exit_bid, short_exit_ask)
        long_exit_fill  = _fill_sell(long_exit_bid, long_exit_ask)
        exit_debit = max(round(short_exit_fill - long_exit_fill, 4), 0)

        # Commission: 4 legs (entry + exit, each 2 legs)
        comm = _commission_per_leg(1, spy) * 4
        pnl  = round((credit - exit_debit) * 100, 2) - comm
        ...
        trades.append(Trade(
            ...
            vix=vix, is_real=use_real, commission=comm, note=exit_reason,
        ))
...
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
# 12. STRATEGY 2 â€” 5-DTE Momentum Calls
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
...
def run_s2_momentum_calls(
    ...
    for ds in dates:
        ...
        if use_real:
            entry_bid, entry_ask, entry_iv, entry_delta = pricer.real_eod_quote(
                dt_obj, exp, float(strike), 'C')
        else:
            ml = max((16 * 60) - (dt_entry.hour * 60 + dt_entry.minute), 1)
            entry_bid, entry_ask, entry_iv, entry_delta = pricer.synthetic_intraday(
                spy, strike, ml, 'C', dt_obj)

        # Apply slippage: buy call at slipped ask
        entry_fill = _fill_buy(entry_bid, entry_ask)

        if entry_fill <= 0 or entry_fill * 100 > 1000:
            dbg['bad_ask'] += 1
            first_reject.setdefault('bad_ask', f'{ds}: entry_fill={entry_fill:.4f}')
            continue

        dbg['entered'] += 1
        target_price  = entry_fill * 1.15
        stop_price    = entry_fill * 0.80
        max_hold_days = 2
        ...
        for hold_ds in dates:
            ...
            if use_real:
                cur_bid, cur_ask, _, _ = pricer.real_eod_quote(hold_dt, exp, float(strike), 'C')
            else:
                hold_bars = spy_5min.get(hold_ds, [])
                hold_spy  = hold_bars[-1]['c'] if hold_bars else spy
                cur_bid, cur_ask, _, _ = pricer.synthetic_intraday(hold_spy, strike, 30, 'C', hold_dt)

            cur_sell = _fill_sell(cur_bid, cur_ask)
            if cur_sell >= target_price:
                exit_reason, actual_exit_ds = '15% target', hold_ds
                break
            if cur_sell <= stop_price:
        ...
        if use_real:
            exit_bid, exit_ask, _, _ = pricer.real_eod_quote(exit_dt_obj, exp, float(strike), 'C')
        else:
            exit_bars = spy_5min.get(actual_exit_ds, [])
            exit_spy  = exit_bars[-1]['c'] if exit_bars else spy
            exit_bid, exit_ask, _, _ = pricer.synthetic_intraday(exit_spy, strike, 30, 'C', exit_dt_obj)

        exit_fill = _fill_sell(exit_bid, exit_ask) if exit_bid > 0 else entry_fill * 0.5

        # Commission: 2 legs (entry + exit)
        comm = _commission_per_leg(1, spy) * 2
        pnl  = round((exit_fill - entry_fill) * 100, 2) - comm
...
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
# 13. STRATEGY 3 â€” FOMC/CPI Catalyst
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
...
def run_s3_catalyst(
    ...
    for fomc_dt in sorted(FOMC_DATES):
        ...
        if use_real:
            entry_bid, entry_ask, iv, delta = pricer.real_eod_quote(entry_dt, exp, float(strike), 'C')
            exit_bid_raw, exit_ask_raw, _, _ = pricer.real_eod_quote(exit_dt, exp, float(strike), 'C')
            entry_fill = _fill_buy(entry_bid, entry_ask)
            exit_fill  = _fill_sell(exit_bid_raw, exit_ask_raw)
        else:
            iv = vix / 100.0
            T  = max((exp - entry_dt).days / 365.0, 0.0001)
            p  = _bs_price_py(spy_px, strike, T, r, iv, 'c')
            syn_bid, syn_ask = round(p * 0.96, 4), round(p * 1.04, 4)
            entry_fill = _fill_buy(syn_bid, syn_ask)
            ...
            exit_fill = _fill_sell(round(p2 * 0.96, 4), round(p2 * 1.04, 4))

        if entry_fill <= 0:
            continue
        dbg['fomc_entered'] += 1
        target_price = entry_fill * 4.0
        stop_price   = entry_fill * 0.50

        if exit_fill >= target_price:
            exit_reason = '300% target'
        elif exit_fill <= stop_price:
            exit_reason = '50% stop'
        else:
            exit_reason = 'day after FOMC'

        comm = _commission_per_leg(1, spy_px) * 2
        pnl  = round((exit_fill - entry_fill) * 100, 2) - comm
    ...
    # CPI: buy SPY puts 2 days before hot CPI months
    hot_cpi_months = {
    ...
    for cpi_dt in sorted(CPI_DATES):
        ...
        if use_real:
            entry_bid, entry_ask, iv, delta = pricer.real_eod_quote(entry_dt, exp, float(strike), 'P')
            exit_bid_raw, exit_ask_raw, _, _ = pricer.real_eod_quote(exit_dt, exp, float(strike), 'P')
            entry_fill = _fill_buy(entry_bid, entry_ask)
            exit_fill  = _fill_sell(exit_bid_raw, exit_ask_raw)
        else:
            iv = vix / 100.0
...
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
# 14. STRATEGY 4 â€” Earnings Momentum
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•

def _get_earnings_dates(ticker: str, start: date, end: date) -> List[date]:
    # All pickle files here are self-written local caches (no external/untrusted source).
...
def run_s4_earnings(
    ...
    for ticker in EARNINGS_UNIVERSE:
        ...
        # All pickle files here are self-written local caches.
        ...
        for earn_dt in earn_dates:
            ...
            if entry_fill <= 0 or entry_fill * 100 > 2000:
                dbg['bad_ask'] += 1
                continue

            dbg['entered'] += 1
            if exit_fill >= entry_fill * 5.0:
            ...
            comm = _commission_per_leg(1, stock_px) * 2
            pnl  = round((exit_fill - entry_fill) * 100, 2) - comm
            ...
            trades.append(Trade(
                ...
                entry_price=entry_fill, exit_price=exit_fill,
                pnl=pnl, entry_iv=iv, entry_delta=delta,
                vix=vix, is_real=use_real, commission=comm,
...
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
# 15. STRATEGY 5 â€” Iron Condor Harvest
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
...
def run_s5_iron_condor(
    ...
    for ds in dates:
        ...
        # Apply slippage: sell shorts at slipped bid, buy longs at slipped ask
        sc_fill = _fill_sell(sc_bid, sc_ask)
        lc_fill = _fill_buy(lc_bid, lc_ask)
        sp_fill = _fill_sell(sp_bid, sp_ask)
        lp_fill = _fill_buy(lp_bid, lp_ask)
        credit  = round((sc_fill - lc_fill) + (sp_fill - lp_fill), 4)
        if credit <= 0.01:
            dbg['low_credit'] += 1
            first_reject.setdefault('low_credit',
                f'{ds}: credit={credit:.4f}')
            continue
        dbg['entered'] += 1

        entry_iv    = (iv_c + iv_p) / 2.0
        target_exit = round(credit * 0.50, 4)
        stop_exit   = round(credit * 2.5,  4)
        ...
        if use_real:
            sc_ex_bid, sc_ex_ask, _, _ = pricer.intraday_price(spy_exit, short_call_k, ml_exit, 'C', dt_obj, expirations)
            lc_ex_bid, lc_ex_ask, _, _ = pricer.intraday_price(spy_exit, long_call_k,  ml_exit, 'C', dt_obj, expirations)
            sp_ex_bid, sp_ex_ask, _, _ = pricer.intraday_price(spy_exit, short_put_k,  ml_exit, 'P', dt_obj, expirations)
            lp_ex_bid, lp_ex_ask, _, _ = pricer.intraday_price(spy_exit, long_put_k,   ml_exit, 'P', dt_obj, expirations)
        else:
            sc_ex_bid, sc_ex_ask, _, _ = pricer.synthetic_intraday(spy_exit, short_call_k, ml_exit, 'C', dt_obj)
            lc_ex_bid, lc_ex_ask, _, _ = pricer.synthetic_intraday(spy_exit, long_call_k,  ml_exit, 'C', dt_obj)
            sp_ex_bid, sp_ex_ask, _, _ = pricer.synthetic_intraday(spy_exit, short_put_k,  ml_exit, 'P', dt_obj)
            lp_ex_bid, lp_ex_ask, _, _ = pricer.synthetic_intraday(spy_exit, long_put_k,   ml_exit, 'P', dt_obj)

        # Exit slippage: buy-to-close shorts at slipped ask, sell-to-close longs at slipped bid
        sc_ex_fill = _fill_buy(sc_ex_bid, sc_ex_ask)
        lc_ex_fill = _fill_sell(lc_ex_bid, lc_ex_ask)
        sp_ex_fill = _fill_buy(sp_ex_bid, sp_ex_ask)
        lp_ex_fill = _fill_sell(lp_ex_bid, lp_ex_ask)
        exit_debit = max(round((sc_ex_fill - lc_ex_fill) + (sp_ex_fill - lp_ex_fill), 4), 0)

        # Commission: 8 legs (4 entry + 4 exit)
        comm = _commission_per_leg(1, spy) * 8
        pnl  = round((credit - exit_debit) * 100, 2) - comm
...
# -- shared exit helpers for put/call credit spreads --------------------------

def _run_spread_exits(bars, entry_bar, short_k, long_k, right,
                       credit, dt_obj, pricer, expirations, use_real, force_hour=15):
    dt_entry    = datetime.fromtimestamp(entry_bar['t'] / 1000, tz=ET)
    target_exit = round(credit * 0.30, 4)
    stop_exit   = round(credit * 2.0,  4)
    exit_bar    = None
    exit_reason = 'EOD'
    for bar in bars:
        dt_bar = datetime.fromtimestamp(bar['t'] / 1000, tz=ET)
        if dt_bar <= dt_entry:
            continue
        if dt_bar.hour >= force_hour:
            exit_bar, exit_reason = bar, f'{force_hour}:00 force exit'
            break
        ml      = max((16 * 60) - (dt_bar.hour * 60 + dt_bar.minute), 1)
        spy_now = bar['c']
        if use_real:
            _, sc_ask, _, _ = pricer.intraday_price(spy_now, short_k, ml, right, dt_obj, expirations)
            lc_bid, _, _, _ = pricer.intraday_price(spy_now, long_k,  ml, right, dt_obj, expirations)
        else:
            _, sc_ask, _, _ = pricer.synthetic_intraday(spy_now, short_k, ml, right, dt_obj)
            lc_bid, _, _, _ = pricer.synthetic_intraday(spy_now, long_k,  ml, right, dt_obj)
        cur_debit = max(round(sc_ask - lc_bid, 4), 0)
        ...
        if cur_debit >= stop_exit:
            ...
            break
    if exit_bar is None:
        exit_bar, exit_reason = bars[-1], 'EOD close'
    return exit_bar, exit_reason


def _spread_exit_pnl(exit_bar, short_k, long_k, right, credit,
                      dt_obj, pricer, expirations, use_real, spy):
    dt_exit  = datetime.fromtimestamp(exit_bar['t'] / 1000, tz=ET)
    spy_exit = exit_bar['c']
    ml_exit  = max((16 * 60) - (dt_exit.hour * 60 + dt_exit.minute), 1)
    if use_real:
        se_bid, se_ask, _, _ = pricer.intraday_price(spy_exit, short_k, ml_exit, right, dt_obj, expirations)
        le_bid, le_ask, _, _ = pricer.intraday_price(spy_exit, long_k,  ml_exit, right, dt_obj, expirations)
    else:
        se_bid, se_ask, _, _ = pricer.synthetic_intraday(spy_exit, short_k, ml_exit, right, dt_obj)
        le_bid, le_ask, _, _ = pricer.synthetic_intraday(spy_exit, long_k,  ml_exit, right, dt_obj)
    exit_debit = max(round(_fill_buy(se_bid, se_ask) - _fill_sell(le_bid, le_ask), 4), 0)
    comm = _commission_per_leg(1, spy) * 4
    pnl  = round((credit - exit_debit) * 100, 2) - comm
    return exit_debit, comm, pnl


# -- STRATEGY 6: R3A Monday-Only Put Spread -----------------------------------

def run_s6_monday_put(
        spy_5min: dict, vix_daily: dict, spy_daily: dict,
        pricer: OptionPricer, expirations: List[date],
        use_real: bool, dates: List[str]) -> List[Trade]:
    ...
    for ds in dates:
        ...
        if len(bars) < 8:
            dbg['few_bars'] += 1
            continue
        exp = _same_day_exp(dt_obj, expirations) or _find_expiration(dt_obj, 0, 1, expirations)
        if exp is None:
            dbg['no_exp'] += 1
            first_reject.setdefault('no_exp', f'{ds}: no 0-1 DTE expiration')
            continue
        ...
        for bar in bars:
            dt_bar = datetime.fromtimestamp(bar['t'] / 1000, tz=ET)
            if (dt_bar.hour == 10 and dt_bar.minute >= 30) or dt_bar.hour > 10:
                if dt_bar.hour < 11:
                    entry_bar = bar
                break
        if entry_bar is None:
            dbg['no_entry_bar'] += 1
            first_reject.setdefault('no_entry_bar', f'{ds}: no bar in 10:30-11:00')
            continue
        ...
        short_k = round(pricer.strike_for_delta(0.20, spy, dt_obj, exp, 'P', expirations, use_real=use_real) / 0.5) * 0.5
        long_k  = short_k - 2.0

        if use_real:
            sh_bid, sh_ask, entry_iv, entry_delta = pricer.intraday_price(spy, short_k, mins_left, 'P', dt_obj, expirations)
            lo_bid, lo_ask, _, _ = pricer.intraday_price(spy, long_k, mins_left, 'P', dt_obj, expirations)
        else:
            sh_bid, sh_ask, entry_iv, entry_delta = pricer.synthetic_intraday(spy, short_k, mins_left, 'P', dt_obj)
            lo_bid, lo_ask, _, _ = pricer.synthetic_intraday(spy, long_k, mins_left, 'P', dt_obj)

        credit = round(_fill_sell(sh_bid, sh_ask) - _fill_buy(lo_bid, lo_ask), 4)
        if credit <= 0.02:
            dbg['low_credit'] += 1
            first_reject.setdefault('low_credit', f'{ds}: credit={credit:.4f}')
            continue
        dbg['entered'] += 1

        exit_bar, exit_reason = _run_spread_exits(bars, entry_bar, short_k, long_k, 'P',
                                                   credit, dt_obj, pricer, expirations, use_real)
        exit_debit, comm, pnl = _spread_exit_pnl(exit_bar, short_k, long_k, 'P',
                                                   credit, dt_obj, pricer, expirations, use_real, spy)
        trades.append(Trade(strategy='S6_MondayPut', date=ds, ticker='SPY',
                            direction='credit_spread_put', entry_date=ds, exit_date=ds,
                            entry_price=credit, exit_price=exit_debit, pnl=pnl,
                            entry_iv=entry_iv, entry_delta=entry_delta,
                            vix=vix, is_real=use_real, commission=comm, note=exit_reason))

    print(f'  S6 debug ({"real" if use_real else "synth"}): '
          f'days={dbg["days"]} weekday={dbg["weekday"]} vix={dbg["vix"]} '
          f'few_bars={dbg["few_bars"]} no_exp={dbg["no_exp"]} '
          f'no_entry_bar={dbg["no_entry_bar"]} low_credit={dbg["low_credit"]} '
          f'entered={dbg["entered"]} trades={len(trades)}')
...
# -- STRATEGY 7: R3C Bear Call Spread (SPY below VWAP at entry) ---------------

def run_s7_bear_call(
        spy_5min: dict, vix_daily: dict, spy_daily: dict,
        pricer: OptionPricer, expirations: List[date],
        use_real: bool, dates: List[str]) -> List[Trade]:

    trades = []
    dbg = {'days': 0, 'vix': 0, 'few_bars': 0, 'no_exp': 0,
           'no_entry_bar': 0, 'above_vwap': 0, 'low_credit': 0, 'entered': 0}
    first_reject: dict = {}
    ...
    for ds in dates:
        ...
        for bar in bars:
            ...
            if dt_bar.hour == 10:
                ...
                if bar['c'] < vwap_now:
                    ...
                    dbg['above_vwap'] += 1
                    first_reject.setdefault('above_vwap',
                        f'{ds}: spy={bar["c"]:.2f} >= vwap={vwap_now:.2f}')
                break
            elif dt_bar.hour > 10:
                break

        if entry_bar is None:
            if not any(datetime.fromtimestamp(b['t'] / 1000, tz=ET).hour == 10 for b in bars):
                dbg['no_entry_bar'] += 1
                first_reject.setdefault('no_entry_bar', f'{ds}: no 10:xx bar')
            continue
        ...
        short_k = round(pricer.strike_for_delta(0.20, spy, dt_obj, exp, 'C', expirations, use_real=use_real) / 0.5) * 0.5
        long_k  = short_k + 2.0

        if use_real:
            sh_bid, sh_ask, entry_iv, entry_delta = pricer.intraday_price(spy, short_k, mins_left, 'C', dt_obj, expirations)
            lo_bid, lo_ask, _, _ = pricer.intraday_price(spy, long_k, mins_left, 'C', dt_obj, expirations)
        else:
            sh_bid, sh_ask, entry_iv, entry_delta = pricer.synthetic_intraday(spy, short_k, mins_left, 'C', dt_obj)
            lo_bid, lo_ask, _, _ = pricer.synthetic_intraday(spy, long_k, mins_left, 'C', dt_obj)
        ...
        exit_bar, exit_reason = _run_spread_exits(bars, entry_bar, short_k, long_k, 'C',
                                                   credit, dt_obj, pricer, expirations, use_real)
        exit_debit, comm, pnl = _spread_exit_pnl(exit_bar, short_k, long_k, 'C',
                                                   credit, dt_obj, pricer, expirations, use_real, spy)
        trades.append(Trade(strategy='S7_BearCall', date=ds, ticker='SPY',
                            direction='credit_spread_call', entry_date=ds, exit_date=ds,
                            entry_price=credit, exit_price=exit_debit, pnl=pnl,
                            entry_iv=entry_iv, entry_delta=entry_delta,
                            vix=vix, is_real=use_real, commission=comm, note=exit_reason))

    print(f'  S7 debug ({"real" if use_real else "synth"}): '
          f'days={dbg["days"]} vix={dbg["vix"]} few_bars={dbg["few_bars"]} '
          f'no_exp={dbg["no_exp"]} no_entry_bar={dbg["no_entry_bar"]} '
          f'above_vwap={dbg["above_vwap"]} low_credit={dbg["low_credit"]} '
          f'entered={dbg["entered"]} trades={len(trades)}')
    if first_reject:
        for k, v in first_reject.items():
            print(f'    first reject [{k}]: {v}')
...
# -- STRATEGY 8: R3E Wednesday Iron Condor (VIX 13-18, delta 0.16) ------------

def run_s8_wednesday_ic(
        spy_5min: dict, vix_daily: dict, spy_daily: dict,
        pricer: OptionPricer, expirations: List[date],
        use_real: bool, dates: List[str]) -> List[Trade]:
    ...
    for ds in dates:
        ...
        sc_k = round(pricer.strike_for_delta(0.16, spy, dt_obj, exp, 'C', expirations, use_real=use_real) / 0.5) * 0.5
        sp_k = round(pricer.strike_for_delta(0.16, spy, dt_obj, exp, 'P', expirations, use_real=use_real) / 0.5) * 0.5
        lc_k = sc_k + 2.0
        lp_k = sp_k - 2.0

        def _q(right, strike, _spy=spy, _ml=mins_left):
            if use_real:
                return pricer.intraday_price(_spy, strike, _ml, right, dt_obj, expirations)
            return pricer.synthetic_intraday(_spy, strike, _ml, right, dt_obj)

        sc_bid, sc_ask, iv_c, _  = _q('C', sc_k)
        lc_bid, lc_ask, _,   _  = _q('C', lc_k)
        sp_bid, sp_ask, iv_p, d  = _q('P', sp_k)
        lp_bid, lp_ask, _,   _  = _q('P', lp_k)

        credit = round((_fill_sell(sc_bid, sc_ask) - _fill_buy(lc_bid, lc_ask)) +
                       (_fill_sell(sp_bid, sp_ask) - _fill_buy(lp_bid, lp_ask)), 4)
        if credit <= 0.01:
            dbg['low_credit'] += 1
            first_reject.setdefault('low_credit', f'{ds}: credit={credit:.4f}')
            continue
        ...
        for bar in bars:
            ...
            def _qe(right, strike, _spy=spy_now, _ml=ml):
                if use_real:
                    return pricer.intraday_price(_spy, strike, _ml, right, dt_obj, expirations)
                return pricer.synthetic_intraday(_spy, strike, _ml, right, dt_obj)

            _, sc_n, _, _ = _qe('C', sc_k)
            lc_n, _, _, _ = _qe('C', lc_k)
            _, sp_n, _, _ = _qe('P', sp_k)
            lp_n, _, _, _ = _qe('P', lp_k)
            cur_debit = max(round((sc_n - lc_n) + (sp_n - lp_n), 4), 0)
            if cur_debit <= target_exit:
                exit_bar, exit_reason = bar, '50% target'
                break
            if cur_debit >= stop_exit:
        ...
        def _qx(right, strike, _spy=spy_exit, _ml=ml_exit):
            if use_real:
                return pricer.intraday_price(_spy, strike, _ml, right, dt_obj, expirations)
            return pricer.synthetic_intraday(_spy, strike, _ml, right, dt_obj)

        sc_xb, sc_xa, _, _ = _qx('C', sc_k)
        lc_xb, lc_xa, _, _ = _qx('C', lc_k)
        sp_xb, sp_xa, _, _ = _qx('P', sp_k)
        lp_xb, lp_xa, _, _ = _qx('P', lp_k)
        exit_debit = max(round((_fill_buy(sc_xb, sc_xa) - _fill_sell(lc_xb, lc_xa)) +
                               (_fill_buy(sp_xb, sp_xa) - _fill_sell(lp_xb, lp_xa)), 4), 0)
        comm = _commission_per_leg(1, spy) * 8
        pnl  = round((credit - exit_debit) * 100, 2) - comm

        trades.append(Trade(strategy='S8_WedCondor', date=ds, ticker='SPY',
                            direction='iron_condor', entry_date=ds, exit_date=ds,
                            entry_price=credit, exit_price=exit_debit, pnl=pnl,
                            entry_iv=entry_iv, entry_delta=d,
                            vix=vix, is_real=use_real, commission=comm, note=exit_reason))

    print(f'  S8 debug ({"real" if use_real else "synth"}): '
          f'days={dbg["days"]} weekday={dbg["weekday"]} vix={dbg["vix"]} '
          f'few_bars={dbg["few_bars"]} no_exp={dbg["no_exp"]} '
          f'no_entry_bar={dbg["no_entry_bar"]} low_credit={dbg["low_credit"]} '
          f'entered={dbg["entered"]} trades={len(trades)}')
...
# -- STRATEGY 9: R3D Credit Spread Mon/Fri, skip FOMC +/-2 days ---------------
...
def run_s9_nofomc(
    ...
    trades = []
    dbg = {'days': 0, 'weekday': 0, 'vix': 0, 'fomc': 0,
           'few_bars': 0, 'no_exp': 0, 'no_entry_bar': 0, 'low_credit': 0, 'entered': 0}
    first_reject: dict = {}
    ...
    for ds in dates:
        ...
        if not (15.0 <= vix <= 22.0):
            ...
            first_reject.setdefault('vix', f'{ds}: vix={vix:.1f}')
            continue
        if any(abs((dt_obj - fd).days) <= 2 for fd in FOMC_DATES):
            dbg['fomc'] += 1
            first_reject.setdefault('fomc', f'{ds}: within 2 days of FOMC')
            continue
        bars = spy_5min.get(ds, [])
        if len(bars) < 8:
            dbg['few_bars'] += 1
        ...
        exit_debit, comm, pnl = _spread_exit_pnl(exit_bar, short_k, long_k, 'P',
                                                   credit, dt_obj, pricer, expirations, use_real, spy)
        trades.append(Trade(strategy='S9_NoFOMC', date=ds, ticker='SPY',
                            direction='credit_spread_put', entry_date=ds, exit_date=ds,
                            entry_price=credit, exit_price=exit_debit, pnl=pnl,
                            entry_iv=entry_iv, entry_delta=entry_delta,
                            vix=vix, is_real=use_real, commission=comm, note=exit_reason))

    print(f'  S9 debug ({"real" if use_real else "synth"}): '
          f'days={dbg["days"]} weekday={dbg["weekday"]} vix={dbg["vix"]} fomc={dbg["fomc"]} '
          f'few_bars={dbg["few_bars"]} no_exp={dbg["no_exp"]} '
          f'no_entry_bar={dbg["no_entry_bar"]} low_credit={dbg["low_credit"]} '
          f'entered={dbg["entered"]} trades={len(trades)}')
    if first_reject:
...
# -- STRATEGY 10: R10 Tuesday Put Spread (SPY above MA50 AND VWAP) ------------

def run_s10_tuesday_vwap(
        spy_5min: dict, vix_daily: dict, spy_daily: dict,
        pricer: OptionPricer, expirations: List[date],
        use_real: bool, dates: List[str]) -> List[Trade]:
    ...
    trades   = []
    ma50     = _load_ma50(spy_daily)
    dbg      = {'days': 0, 'weekday': 0, 'vix': 0, 'no_ma50': 0, 'few_bars': 0,
                'no_exp': 0, 'no_window': 0, 'below_filter': 0, 'low_credit': 0, 'entered': 0}
    first_reject: dict = {}
    ...
    for ds in dates:
        ...
        for bar in bars:
            bars_so_far.append(bar)
            dt_bar = datetime.fromtimestamp(bar['t'] / 1000, tz=ET)
            if dt_bar.hour > 10 or (dt_bar.hour == 10 and dt_bar.minute >= 45):
                found_window = True
                vwap_now = _vwap(bars_so_far)
                spy_c    = bar['c']
                if spy_c > ma50_val and spy_c > vwap_now:
                    entry_bar = bar
                else:
                    dbg['below_filter'] += 1
                    first_reject.setdefault('below_filter',
                        f'{ds}: spy={spy_c:.2f} ma50={ma50_val:.2f} vwap={vwap_now:.2f}')
                break
        if not found_window:
            dbg['no_window'] += 1
            first_reject.setdefault('no_window', f'{ds}: no bar at/after 10:45')
        if entry_bar is None:
            continue
        ...
        exit_debit, comm, pnl = _spread_exit_pnl(exit_bar, short_k, long_k, 'P',
                                                   credit, dt_obj, pricer, expirations, use_real, spy)
        trades.append(Trade(strategy='S10_TuesdayVWAP', date=ds, ticker='SPY',
                            direction='credit_spread_put', entry_date=ds, exit_date=ds,
                            entry_price=credit, exit_price=exit_debit, pnl=pnl,
                            entry_iv=entry_iv, entry_delta=entry_delta,
                            vix=vix, is_real=use_real, commission=comm, note=exit_reason))

    print(f'  S10 debug ({"real" if use_real else "synth"}): '
          f'days={dbg["days"]} weekday={dbg["weekday"]} vix={dbg["vix"]} '
          f'no_ma50={dbg["no_ma50"]} few_bars={dbg["few_bars"]} no_exp={dbg["no_exp"]} '
          f'no_window={dbg["no_window"]} below_filter={dbg["below_filter"]} '
          f'low_credit={dbg["low_credit"]} entered={dbg["entered"]} trades={len(trades)}')
    if first_reject:
        for k, v in first_reject.items():
            print(f'    first reject [{k}]: {v}')
    return trades
...
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
# 17. PARAMETER STABILITY  (UPGRADE 3)
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•

def run_parameter_stability(spy_5min, vix_daily, spy_daily, pricer, expirations,
                             use_real, oos_dates):
    """Parameter sensitivity sweep for S1 on OOS dates."""
    results = []

    def _run(label, group, **kwargs):
        defaults = dict(vix_min=15.0, vix_max=22.0, target_delta=0.20,
                        entry_hour_start=10, entry_minute_start=0, entry_hour_end=11)
        defaults.update(kwargs)
        trades = run_s1_credit_spread(
            spy_5min, vix_daily, spy_daily, pricer, expirations,
            use_real, oos_dates, **defaults)
    ...
    _run('VIX [13-20]',            'VIX',   vix_min=13,  vix_max=20)
    _run('VIX [14-21]',            'VIX',   vix_min=14,  vix_max=21)
    _run('VIX [15-22] *baseline*', 'VIX',   vix_min=15,  vix_max=22)
    _run('VIX [16-23]',            'VIX',   vix_min=16,  vix_max=23)
    _run('VIX [17-24]',            'VIX',   vix_min=17,  vix_max=24)

    _run('Delta=0.15',             'Delta', target_delta=0.15)
    _run('Delta=0.18',             'Delta', target_delta=0.18)
    _run('Delta=0.20 *baseline*',  'Delta', target_delta=0.20)
    _run('Delta=0.22',             'Delta', target_delta=0.22)
    _run('Delta=0.25',             'Delta', target_delta=0.25)

    _run('Entry [9:30-10:00]',          'Time', entry_hour_start=9,  entry_minute_start=30, entry_hour_end=10)
    _run('Entry [10:00-11:00] *base*',  'Time', entry_hour_start=10, entry_minute_start=0,  entry_hour_end=11)
    _run('Entry [10:30-11:30]',         'Time', entry_hour_start=10, entry_minute_start=30, entry_hour_end=11)
    _run('Entry [11:00-12:00]',         'Time', entry_hour_start=11, entry_minute_start=0,  entry_hour_end=12)

    return results


# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
# 18. CRASH PERIOD ANALYSIS  (UPGRADE 4)
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•

def run_crash_analysis(pricer, expirations, spy_5min, vix_daily, spy_daily,
                        all_dates, theta_ok):
    results = {}

    # Periods within main backtest range: use existing data
    for key in ('2022_bear', '2024_aug'):
        start, end, label = CRASH_PERIODS[key]
        crash_dates = [d for d in all_dates
                       if start.isoformat() <= d <= end.isoformat()]
        trades = run_s1_credit_spread(spy_5min, vix_daily, spy_daily, pricer,
                                      expirations, theta_ok, crash_dates)
        results[key] = {
            'label':    label,
            'stats':    _calc_stats(trades),
            'n_trades': len(trades),
            'mode':     'real' if theta_ok else 'synthetic',
        }

    # Periods outside main backtest range: load fresh data synthetically
    for key in ('2018_Q4', '2020_march'):
        start, end, label = CRASH_PERIODS[key]
        try:
            c_vix = _load_vix(start, end)
            c_spy = _load_spy_daily(start, end)
            try:
                c_5min = _load_alpaca_5min(start, end)
                if not c_5min:
                    c_5min = _synth_5min_from_daily(c_spy)
            except Exception:
                c_5min = _synth_5min_from_daily(c_spy)
            c_exps   = _synthetic_expirations(start, end)
            # cache_only=True: only self-written local files, no untrusted source
            c_theta  = ThetaCache(DATA_DIR, {}, cache_only=True)
            c_pricer = OptionPricer(c_theta, c_spy, c_vix)
            c_dates  = sorted(c_spy.keys())
            trades   = run_s1_credit_spread(c_5min, c_vix, c_spy, c_pricer,
                                            c_exps, False, c_dates)
            results[key] = {
                'label':    label,
                'stats':    _calc_stats(trades),
                'n_trades': len(trades),
                'mode':     'synthetic (extended)',
            }
...
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
# 19. CONFIDENCE SCORE  (UPGRADE 5)
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•

def _confidence_score(full_s, full_pp, full_grade,
                       oos_s, oos_pp, oos_grade,
                       blind_s, blind_pp,
                       stability_results, crash_results) -> dict:
    breakdown = {}
    total = 0

    # 1. Full-period grade
    pts = 20 if full_grade == 'A' else (10 if full_grade == 'B' else 0)
    breakdown['Grade A (full period)'] = (pts, 20, f'Grade {full_grade}')
    total += pts

    # 2. OOS grade
    pts = 20 if oos_grade == 'A' else (10 if oos_grade == 'B' else 0)
    breakdown['Grade A (OOS)'] = (pts, 20, f'Grade {oos_grade}')
    total += pts

    # 3. Blind year kill criteria
    blind_pass = (blind_s['n'] >= 3 and blind_s['wr'] >= 45.0
                  and blind_s['total_pnl'] > -100)
    pts = 20 if blind_pass else 0
    blind_detail = (f"N={blind_s['n']} WR={blind_s['wr']:.1f}% "
                    f"P&L=${blind_s['total_pnl']:+.0f}")
    breakdown['Blind 2025 passes kill criteria'] = (
        pts, 20, ('PASS â€” ' if blind_pass else 'FAIL â€” ') + blind_detail)
    total += pts

    # 4. Parameter stability
    fail_grades  = {'D', 'F'}
    below_c      = [r for r in stability_results if r['grade'] in fail_grades]
    stable       = len(below_c) == 0
    pts = 15 if stable else (7 if len(below_c) <= 2 else 0)
    breakdown['Parameter stability'] = (
        pts, 15,
        f'{len(below_c)} of {len(stability_results)} variations below C')
    total += pts

    # 5. Crash survival
    crash_fails = []
    for key, cr in crash_results.items():
        st = cr['stats']
        if st['n'] > 0 and st['total_pnl'] < -200:
            crash_fails.append(f"{key}(${st['total_pnl']:+.0f})")
    crash_ok = len(crash_fails) == 0
    pts = 15 if crash_ok else 0
    breakdown['Crash period survival'] = (
        pts, 15, 'PASS' if crash_ok else f'FAIL: {crash_fails}')
    total += pts

    # 6. Bootstrap
    pts = 10 if full_pp >= 95 else (5 if full_pp >= 80 else 0)
    breakdown['Bootstrap P(>0) > 95%'] = (pts, 10, f'{full_pp:.1f}%')
    total += pts
...
def _simple_confidence_score(trades) -> dict:
    """4-dimension, 25 pts each — used for S6-S10 (no stability/crash sweep)."""
    ...
    pp     = _bootstrap_p_positive(trades)
    grade  = _grade(s, pp)
    pts    = 25 if grade == 'A' else (12 if grade == 'B' else 0)
    breakdown['Grade A (full period)'] = (pts, 25, f'Grade {grade}')
    total += pts

    # 2. OOS grade (trades after SPLIT_DATE, excluding BLIND_YEAR)
    oos = [t for t in trades if SPLIT_DATE.isoformat() <= t.entry_date
           and t.entry_date[:4] != str(BLIND_YEAR)]
    oos_s    = _calc_stats(oos)
    oos_pp   = _bootstrap_p_positive(oos)
    oos_g    = _grade(oos_s, oos_pp)
    pts      = 25 if oos_g == 'A' else (12 if oos_g == 'B' else 0)
    breakdown['Grade A (OOS)'] = (pts, 25, f'Grade {oos_g}')
    total += pts

    # 3. Blind year
    blind   = [t for t in trades if t.entry_date[:4] == str(BLIND_YEAR)]
    blind_s = _calc_stats(blind)
    blind_pass = (blind_s['n'] >= 3 and blind_s['wr'] >= 45.0
                  and blind_s['total_pnl'] > -100)
    pts = 25 if blind_pass else 0
    blind_detail = (f"N={blind_s['n']} WR={blind_s['wr']:.1f}% "
                    f"P&L=${blind_s['total_pnl']:+.0f}")
    breakdown['Blind 2025 passes kill criteria'] = (
        pts, 25, ('PASS - ' if blind_pass else 'FAIL - ') + blind_detail)
    total += pts

    # 4. Bootstrap
    pts = 25 if pp >= 95 else (12 if pp >= 80 else 0)
    breakdown['Bootstrap P(>0) > 95%'] = (pts, 25, f'{pp:.1f}%')
    total += pts

    return {'total': total, 'max': 100, 'breakdown': breakdown}

STRAT_NAMES = {
    'S1_CreditSpread':  'S1 â€” Credit Spread        (0DTE put spread, Mon/Fri, VIX 15-22)',
    'S2_5DTE_Momentum': 'S2 â€” 5-DTE Momentum Calls (morning bias, HH+vol+RSI)',
    'S3_FOMC_Catalyst': 'S3 â€” FOMC Catalyst         (calls 2 days pre-FOMC)',
    'S3_CPI_Catalyst':  'S3 â€” CPI Catalyst          (puts 2 days pre-hot-CPI)',
    'S4_Earnings':      'S4 â€” Earnings Momentum     (calls 5 days pre-earnings)',
    'S5_IronCondor':    'S5 â€” Iron Condor Harvest   (0DTE, VIX>20, IVR>=50)',
}
...
def _strat_section_90pct(label, real_trades, synth_trades, vix_daily, SEP, sep):
    lines = [SEP, f'  {label}', sep]
    for tag, trades in [('REAL DATA (90PCT)', real_trades), ('SYNTHETIC (90PCT)', synth_trades)]:
        s      = _calc_stats(trades)
        pp     = _bootstrap_p_positive(trades)
        grade  = _grade(s, pp)
        ...
        oos_trades = [t for t in trades if SPLIT_DATE.isoformat() <= t.entry_date
                      and t.entry_date[:4] != str(BLIND_YEAR)]
        ...
        lines += [
            f'  [{tag}]',
            f'  Total commissions deducted: ${total_comm:,.2f}',
            f'  Full period  : N={s["n"]:>4}  WR={s["wr"]:>5.1f}%  '
            f'P&L=${s["total_pnl"]:>+9.2f}  PF={_fmt_pf(s)}  '
            f'Sharpe={s["sharpe"]:>5.2f}  Boot={pp:.0f}%  Grade={grade}',
            f'  In-sample    : N={is_s["n"]:>4}  WR={is_s["wr"]:>5.1f}%  '
            f'P&L=${is_s["total_pnl"]:>+9.2f}  PF={_fmt_pf(is_s)}',
            f'  Out-of-sample: N={oos_s["n"]:>4}  WR={oos_s["wr"]:>5.1f}%  '
            f'P&L=${oos_s["total_pnl"]:>+9.2f}  PF={_fmt_pf(oos_s)}',
        ...
        for yr, ys in regime.items():
            lines.append(f'    {yr}: N={ys["n"]:>3}  WR={ys["wr"]:>5.1f}%  '
                         f'P&L=${ys["total_pnl"]:>+8.2f}  PF={_fmt_pf(ys)}')
        lines.append('')
    return lines


def build_report_90pct(all_real, all_synth, vix_daily, n_days,
                        stability_results, crash_results, confidence,
                        new_confidences=None) -> str:
    SEP = '=' * 80
    sep = '-' * 80
    now = datetime.now().strftime('%Y-%m-%d %H:%M:%S')

    # S1 splits for confidence section
    s1_real    = [t for t in all_real if t.strategy == 'S1_CreditSpread']
    s1_full_s  = _calc_stats(s1_real)
    s1_full_pp = _bootstrap_p_positive(s1_real)
    s1_full_g  = _grade(s1_full_s, s1_full_pp)
    s1_oos     = [t for t in s1_real if SPLIT_DATE.isoformat() <= t.entry_date
                  and t.entry_date[:4] != str(BLIND_YEAR)]
    s1_oos_s   = _calc_stats(s1_oos)
    s1_oos_pp  = _bootstrap_p_positive(s1_oos)
    s1_oos_g   = _grade(s1_oos_s, s1_oos_pp)
    s1_blind   = [t for t in s1_real if t.entry_date[:4] == str(BLIND_YEAR)]
    s1_blind_s = _calc_stats(s1_blind)

    spu = (COMMISSION_BASE + 550 * 100 * SEC_FEE_RATE
           + 100 * FINRA_FEE_RATE + EXCHANGE_FEE)

    lines = [
        SEP,
        '  90% CONFIDENCE BACKTEST â€” SPY OPTIONS (ThetaData EOD bid/ask)',
        f'  Period   : {BT_START} to {BT_END}  ({n_days} trading days)',
        f'  Split    : 60% in-sample (<{SPLIT_DATE}) | 40% OOS | blind {BLIND_YEAR}',
        f'  Generated: {now}',
        sep,
        '  PRICING MODEL:',
        f'    Commission: ${COMMISSION_BASE:.2f} base + SEC(SPY*{SEC_FEE_RATE*100:.4f}%) + '
        f'FINRA(${FINRA_FEE_RATE*100:.4f}/share) + Exchange(${EXCHANGE_FEE:.2f})',
        f'                ~${spu:.2f}/contract/leg at SPY~$550 (entry AND exit)',
        f'    Slippage  : Buy at ask+15%spread | Sell at bid-15%spread',
        SEP, '',
    ]
    ...
    strategy_groups = [
        ...
        ('S6_MondayPut',     'S6 - Monday-Only Put Spread (0DTE | Mon | 10:30 | VIX 15-22)'),
        ('S7_BearCall',      'S7 - Bear Call Spread (0DTE | any day | SPY<VWAP | VIX 15-22)'),
        ('S8_WedCondor',     'S8 - Wednesday Iron Condor (0DTE | Wed | 10:30 | VIX 13-18)'),
        ('S9_NoFOMC',        'S9 - No-FOMC Credit Spread (0DTE | Mon/Fri | skip FOMC +/-2)'),
        ('S10_TuesdayVWAP',  'S10 - Tuesday VWAP+MA50 (0DTE | Tue | 10:45 | SPY>MA50&VWAP)'),
    ]
    for strat_key, strat_label in strategy_groups:
        ...
        lines += _strat_section_90pct(strat_label, rt, st, vix_daily, SEP, sep)

    # â”€â”€ Parameter Stability â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    lines += [SEP, '  PARAMETER STABILITY â€” S1 Credit Spread (OOS period)', sep, '']
    for grp in ('VIX', 'Delta', 'Time'):
        lines.append(f'  {grp} SENSITIVITY:')
        hdr = (f'  {"Variation":<32} {"N":>5} {"WR%":>7} {"P&L":>10} '
               f'{"PF":>6} {"Grade":>6}  Overfitted?')
        lines.append(hdr)
        lines.append('  ' + '-' * (len(hdr) - 2))
        for r in [x for x in stability_results if x['group'] == grp]:
            pf_str   = f"{r['pf']:5.2f}" if r['pf'] != float('inf') else '  inf'
            over     = '  *** OVERFITTED ***' if r['grade'] in ('D', 'F') else ''
            base_mrk = ' <-- baseline' if r['is_baseline'] else ''
            lines.append(
                f'  {r["label"]:<32} {r["n"]:>5} {r["wr"]:>6.1f}% '
                f'${r["total_pnl"]:>+9.0f} {pf_str} {r["grade"]:>6}'
                f'{over}{base_mrk}')
        lines.append('')

    below_c_count = sum(1 for r in stability_results if r['grade'] in ('D', 'F'))
    if below_c_count == 0:
        stab_verdict = f'STABLE â€” 0 of {len(stability_results)} variations dropped below grade C'
    else:
        stab_verdict = (f'OVERFITTED â€” {below_c_count} of {len(stability_results)} '
                        f'variations dropped below grade C')
    lines += [f'  STABILITY VERDICT: {stab_verdict}', '', SEP, '']

    # â”€â”€ Crash Period Analysis â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    lines += [SEP, '  CRASH PERIOD ANALYSIS â€” S1 Credit Spread', sep, '']
    lines.append(f'  {"Period":<46} {"N":>5} {"WR%":>7} {"P&L":>10}  Note')
    lines.append('  ' + '-' * 80)
    for key in ('2018_Q4', '2020_march', '2022_bear', '2024_aug'):
        if key not in crash_results:
            continue
        cr    = crash_results[key]
        st    = cr['stats']
        label = CRASH_PERIODS[key][2][:45]
        lines.append(
            f'  {label:<46} {st["n"]:>5} {st["wr"]:>6.1f}% '
            f'${st["total_pnl"]:>+9.0f}  {cr["mode"]}')
    lines += [
        '',
        '  NOTE: 0 trades during extreme VIX (>22) periods = strategy correctly sidestepped.',
        '', SEP, '',
    ]
    ...
    lines += [SEP, '  CONFIDENCE SCORE â€” S1 Credit Spread', sep, '']
    lines.append(f'  {"Component":<42} {"Earned":>7} {"Max":>5}  Detail')
    lines.append('  ' + '-' * 72)
    for comp, (earned, max_pts, detail) in confidence['breakdown'].items():
        lines.append(f'  {comp:<42} {earned:>7} {max_pts:>5}  {detail}')
    lines += [
        '  ' + '-' * 72,
        f'  {"TOTAL CONFIDENCE SCORE:":<42} {confidence["total"]:>5} / {confidence["max"]}',
        '',
        '  INTERPRETATION:',
        '    90-100 : Production ready â€” extremely high confidence',
        '    75-89  : Strong â€” minor concerns, monitor live',
        '    60-74  : Moderate â€” paper trade 3+ months before live',
        '    45-59  : Weak â€” significant overfitting risk',
        '    <45    : Fail â€” do not trade live',
        '',
    ]
    score = confidence['total']
    if score >= 90:
        interp = 'PRODUCTION READY â€” extremely high confidence'
    elif score >= 75:
        interp = 'STRONG â€” minor concerns, monitor live before scaling'
    elif score >= 60:
        interp = 'MODERATE â€” paper trade 3+ months before going live'
    elif score >= 45:
        interp = 'WEAK â€” significant overfitting risk, do not trade live'
    else:
        interp = 'FAIL â€” strategy does not meet 90% confidence threshold'
    lines += [f'  CONFIDENCE VERDICT: {score}/100 â€” {interp}', '', SEP, '']

    # -- S6-S10 Confidence Scores -------------------------------------------
    if new_confidences:
        lines += [SEP, '  CONFIDENCE SCORES -- S6-S10 (4-dimension, 25 pts each)', sep, '']
        for sname, sc in new_confidences.items():
            lines.append(f'  {sname}:  {sc["total"]}/100')
            lines.append(f'  {"Component":<42} {"Earned":>7} {"Max":>5}  Detail')
            lines.append('  ' + '-' * 72)
            for comp, (earned, max_pts, detail) in sc['breakdown'].items():
                lines.append(f'  {comp:<42} {earned:>7} {max_pts:>5}  {detail}')
            score6 = sc['total']
            if score6 >= 75:
                interp6 = 'STRONG'
    ...
    lines += [SEP, '  STRATEGY SUMMARY (with full commission + slippage)', sep, '']
    hdr = (f'  {"Strategy":<28} {"N":>6} {"WR%":>7} {"P&L":>12} {"PF":>7} '
           f'{"Sharpe":>8} {"Grade":>6}')
    lines.append(hdr)
    lines.append('  ' + '-' * (len(hdr) - 2))
    for strat_key, short_name in [
        ...
        ('S6_MondayPut',     'S6 MondayPut'),
        ('S7_BearCall',      'S7 BearCall'),
        ('S8_WedCondor',     'S8 WedCondor'),
        ('S9_NoFOMC',        'S9 NoFOMC'),
        ('S10_TuesdayVWAP',  'S10 TuesdayVWAP'),
    ]:
        if strat_key == 'S3_both':
            rt = [t for t in all_real if t.strategy.startswith('S3_')]
        else:
        ...
        grade = _grade(rs, pp)
        lines.append(
            f'  {short_name:<28} {rs["n"]:>6} {rs["wr"]:>6.1f}% '
            f'${rs["total_pnl"]:>+10.0f} {_fmt_pf(rs)} '
            f'{rs["sharpe"]:>8.2f} {grade:>6}')
    lines += ['', SEP]
    return '\n'.join(lines)
...
def write_trades_csv(trades: List[Trade], path: Path) -> None:
    ...
    fields = ['strategy', 'is_real', 'date', 'ticker', 'direction',
              ...
              'pnl', 'commission', 'entry_iv', 'entry_delta', 'vix', 'note']
...
def main():
    ...
    W  = 80
    print(f'\n{"=" * W}')
    print('  90% CONFIDENCE BACKTEST  â€”  SPY Options (ThetaData EOD bid/ask)')
    print(f'  Period: {BT_START} to {BT_END}')
    print(f'  Upgrades: Full commission | Improved slippage | Param stability |')
    print(f'            Crash analysis | Confidence scoring')
    print('=' * W)
    ...
    print(f'  {n_days} trading days  |  VIX avg '
          f'{sum(vix_daily.values())/max(len(vix_daily),1):.1f}')
    ...
    print(f'  Found {len(expirations)} cached SPY expiration files')

    pricer = OptionPricer(theta, spy_daily, vix_daily)

    print('\n  Running strategies (REAL DATA + commission + slippage)...')
    print('  [1/10] S1 Credit Spread...')
    s1_real = run_s1_credit_spread(spy_5min, vix_daily, spy_daily, pricer,
                                    expirations, theta_ok, all_dates)
    s1_s = _calc_stats(s1_real)
    print(f'        N={s1_s["n"]}  WR={s1_s["wr"]:.1f}%  P&L=${s1_s["total_pnl"]:+.2f}')

    print('  [2/10] S2 5-DTE Momentum...')
    ...
    print('  [3/10] S3 FOMC/CPI Catalyst...')
    ...
    print('  [4/10] S4 Earnings Momentum...')
    ...
    print('  [6/10] S6 Monday Put Spread...')
    s6_real = run_s6_monday_put(spy_5min, vix_daily, spy_daily, pricer,
                                 expirations, theta_ok, all_dates)
    s6_s = _calc_stats(s6_real)
    print(f'         N={s6_s["n"]}  WR={s6_s["wr"]:.1f}%  P&L=${s6_s["total_pnl"]:+.2f}')

    print('  [7/10] S7 Bear Call Spread...')
    s7_real = run_s7_bear_call(spy_5min, vix_daily, spy_daily, pricer,
                                expirations, theta_ok, all_dates)
    s7_s = _calc_stats(s7_real)
    print(f'         N={s7_s["n"]}  WR={s7_s["wr"]:.1f}%  P&L=${s7_s["total_pnl"]:+.2f}')

    print('  [8/10] S8 Wednesday Iron Condor...')
    s8_real = run_s8_wednesday_ic(spy_5min, vix_daily, spy_daily, pricer,
                                   expirations, theta_ok, all_dates)
    s8_s = _calc_stats(s8_real)
    print(f'         N={s8_s["n"]}  WR={s8_s["wr"]:.1f}%  P&L=${s8_s["total_pnl"]:+.2f}')

    print('  [9/10] S9 No-FOMC Credit Spread...')
    s9_real = run_s9_nofomc(spy_5min, vix_daily, spy_daily, pricer,
                             expirations, theta_ok, all_dates)
    s9_s = _calc_stats(s9_real)
    print(f'         N={s9_s["n"]}  WR={s9_s["wr"]:.1f}%  P&L=${s9_s["total_pnl"]:+.2f}')

    print('  [10/10] S10 Tuesday VWAP+MA50...')
    s10_real = run_s10_tuesday_vwap(spy_5min, vix_daily, spy_daily, pricer,
                                     expirations, theta_ok, all_dates)
    s10_s = _calc_stats(s10_real)
    print(f'          N={s10_s["n"]}  WR={s10_s["wr"]:.1f}%  P&L=${s10_s["total_pnl"]:+.2f}')

    all_real = s1_real + s2_real + s3_real + s4_real + s5_real + s6_real + s7_real + s8_real + s9_real + s10_real
    print(f'\n  REAL TOTAL: {len(all_real)} trades  '
          f'P&L=${sum(t.pnl for t in all_real):+,.2f}')

    print('\n  Running strategies (SYNTHETIC / comparison)...')
    s1_synth = run_s1_credit_spread(spy_5min, vix_daily, spy_daily, pricer,
                                     expirations, False, all_dates)
    ...
    s5_synth = run_s5_iron_condor(spy_5min, vix_daily, spy_daily, pricer,
                                   expirations, False, all_dates)
    s6_synth  = run_s6_monday_put(spy_5min, vix_daily, spy_daily, pricer,
                                   expirations, False, all_dates)
    s7_synth  = run_s7_bear_call(spy_5min, vix_daily, spy_daily, pricer,
                                  expirations, False, all_dates)
    s8_synth  = run_s8_wednesday_ic(spy_5min, vix_daily, spy_daily, pricer,
                                     expirations, False, all_dates)
    s9_synth  = run_s9_nofomc(spy_5min, vix_daily, spy_daily, pricer,
                               expirations, False, all_dates)
    s10_synth = run_s10_tuesday_vwap(spy_5min, vix_daily, spy_daily, pricer,
                                      expirations, False, all_dates)
    all_synth = (s1_synth + s2_synth + s3_synth + s4_synth + s5_synth
                 + s6_synth + s7_synth + s8_synth + s9_synth + s10_synth)
    print(f'  SYNTH TOTAL: {len(all_synth)} trades  '
          f'P&L=${sum(t.pnl for t in all_synth):+,.2f}')

    print('\n  Running parameter stability test (S1, OOS dates)...')
    oos_dates = [d for d in all_dates
                 if SPLIT_DATE.isoformat() <= d
                 and not d.startswith(str(BLIND_YEAR))]
    stability_results = run_parameter_stability(
        spy_5min, vix_daily, spy_daily, pricer, expirations, theta_ok, oos_dates)
    print(f'  Stability: {len(stability_results)} variations tested')

    print('\n  Running crash period analysis (S1)...')
    crash_results = run_crash_analysis(pricer, expirations, spy_5min, vix_daily,
                                       spy_daily, all_dates, theta_ok)
    for key, cr in crash_results.items():
        print(f'    {key}: N={cr["n_trades"]}  '
              f'P&L=${cr["stats"]["total_pnl"]:+.0f}  [{cr["mode"]}]')

    print('\n  Computing confidence scores...')
    s1_full_s   = _calc_stats(s1_real)
    s1_full_pp  = _bootstrap_p_positive(s1_real)
    s1_full_g   = _grade(s1_full_s, s1_full_pp)
    s1_oos_t    = [t for t in s1_real if SPLIT_DATE.isoformat() <= t.entry_date
                   and t.entry_date[:4] != str(BLIND_YEAR)]
    s1_oos_s    = _calc_stats(s1_oos_t)
    s1_oos_pp   = _bootstrap_p_positive(s1_oos_t)
    s1_oos_g    = _grade(s1_oos_s, s1_oos_pp)
    s1_blind_t  = [t for t in s1_real if t.entry_date[:4] == str(BLIND_YEAR)]
    s1_blind_s  = _calc_stats(s1_blind_t)
    s1_blind_pp = _bootstrap_p_positive(s1_blind_t)
    confidence  = _confidence_score(
        s1_full_s,  s1_full_pp,  s1_full_g,
        s1_oos_s,   s1_oos_pp,   s1_oos_g,
        s1_blind_s, s1_blind_pp,
        stability_results, crash_results)
    print(f'  S1 Confidence Score: {confidence["total"]}/100')

    new_confidences = {}
    for sname, strades in [
        ('S6_MondayPut',    s6_real),
        ('S7_BearCall',     s7_real),
        ('S8_WedCondor',    s8_real),
        ('S9_NoFOMC',       s9_real),
        ('S10_TuesdayVWAP', s10_real),
    ]:
        sc = _simple_confidence_score(strades)
        new_confidences[sname] = sc
        print(f'  {sname} Confidence Score: {sc["total"]}/100')

    print('\n  Building report...')
    report = build_report_90pct(all_real, all_synth, vix_daily, n_days,
                                 stability_results, crash_results, confidence,
